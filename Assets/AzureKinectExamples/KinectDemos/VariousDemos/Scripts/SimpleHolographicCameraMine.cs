...
namespace com.rfilkov.components
{
    ...
    ///   each of the views.
    /// </summary>
    class SimpleHolographicCameraMine : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            cam = GameObject.Find("Camera_building").GetComponent<Camera>();
            if (cam)
            {
                ...
                /*
                initialCamPos = building.transform.position;
                initialCamRot = building.transform.rotation;
                initialCamMat = Matrix4x4.TRS(initialCamPos, initialCamRot, Vector3.one);
                initialCamPrjMat = cam.projectionMatrix;
                */
            }
        ...
        void Update()
        {
            ...
            if (kinectManager && kinectManager.IsInitialized())
            {
                ...
                if (kinectManager.IsUserTracked(userId) && kinectManager.IsJointTracked(userId, KinectInterop.JointType.SpineChest))
                {
        ...
        void LateUpdate()
        {
            if (cam)
            {
                if (headPosValid)
                {
                    ...
                    //building.transform.position = camPoseMat.GetColumn(3); //"(0.43, -0.49, -1.05)"
                    //building.transform.rotation = camPoseMat.rotation;//"(0.00000, 0.00000, 0.00000, 1.00000)"
                ...
                else
                {
                    ...
                    /*
                    building.transform.position = initialCamPos;
                    building.transform.rotation = initialCamRot;
                    */
                }
            }
        }
